...
# --------- CONFIG ---------
RESPONSES_MODEL = "gpt-4o-mini"       # planner model
...
Return STRICT JSON ONLY (no prose, no markdown). Follow this schema exactly:
...
def make_user_prompt(topic: str, base_text: str) -> str:
    ref = f"\nREFERENCE TEXT:\n{base_text}\n" if base_text else ""
    return f"""
Create one {duration_min}-minute {lang} lesson for level {level}.
...
Output JSON ONLY, no markdown, no commentary, matching the schema.
{ref}
""".strip()

# --- Robust JSON parsing (in case model adds stray text) ---
def extract_json_block(s: str) -> str:
    # Try a quick clean first (strip code fences)
    s = s.strip()
    ...
    if m:
        return m.group(0)
    return s  # last resort (will likely fail, but error will be shown)
...
def plan_lesson():
    ...
    try:
        resp = client.responses.create(
            ...
            input=messages,              # Responses API accepts this messages-like input
            temperature=0.7,
        )
        ...
        try:
            st.caption("Model content (first 1000 chars):")
            st.code((locals().get("content") or "")[:1000])
        except Exception:
            pass
        st.stop()
...
def assemble_audio(timeline: List[Dict]) -> (AudioSegment, List[str]):
    ...
    for step in timeline:
        ...
        if action in ("prompt", "recall") and voice == "coach" and text_en:
            seg = tts_to_seg(text_en, TTS_VOICE_COACH)
            ...
            seg = tts_to_seg(text_tl, TTS_VOICE_NATIVE)
            lesson += seg
            captions.append(f"NATIVE: {text_tl}")

        if action == "prompt" and step.get("expect_tl"):
            ans = (step.get("expect_tl") or "").strip()
            if ans:
                seg = tts_to_seg(ans, TTS_VOICE_NATIVE)
                lesson += seg
...
if st.button("Generate Lesson", type="primary"):
    ...
    with st.spinner("Synthesizing audio..."):
        audio, caps = assemble_audio(plan.get("timeline", []))
